...
function LoadDistributionPlanner({ config }) {
  const { lineups: initialLineups } = parseConfig(config);
  ...
  const savePlannerState = () => {
    try {
      ...
      localStorage.setItem('pduPlannerState', JSON.stringify(data));
      alert('Planner state saved');
    } catch (err) {
      console.error('Save failed:', err);
      alert('Error saving planner state');
    }
  };

  const loadPlannerState = () => {
    try {
      const raw = localStorage.getItem('pduPlannerState');
      if (!raw) return alert('No saved state found');
      const data = JSON.parse(raw);
      setSelectedLineups(data.selectedLineups || []);
      setPduUsage(data.pduUsage || {});
      setBreakerSelection(data.breakerSelection || {});
      setCustomDistribution(data.customDistribution || []);
      setTargetLoadMW(data.targetLoadMW || 5);
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '1rem' }}>
      ...
      <PlannerHeader
        ...
        savePlannerState={savePlannerState}
        loadPlannerState={loadPlannerState}
      />
